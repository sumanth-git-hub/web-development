...
export default function CountryDetail() {
  ...
  const reUsableFunction = (displayData) => {
    setCountryData({
    ...
    Promise.all(
      displayData.borders.map(async (findBorder) => {
        const res = await fetch(
          `https://restcountries.com/v3.1/alpha/${findBorder}`
        );
