...
export default function ExpenseForm({ prepareExpenses,  inputValue, setInputValue, editingRowId, setEditingRowId}) {

  // lifted this state to app
...
submitButton.current.classList.add("submitButton")
  ...
  const validate = (formData) => {
    ...
    validationConfig[key].some((rule) => {
        ...
        if(rule.type === Number && !/^[0-9.,]*$/.test(value)){
          errorsData[key] = rule.message
          return true
        }
  ...
  const handleSubmit = (e) => {
    ...
    if(editingRowId){
      prepareExpenses((prevState) => prevState.map((findExpense) => {
        if(findExpense.id === editingRowId){
      return {...inputValue, id: editingRowId}
     }
     return findExpense
      } ))
      setInputValue({
  ...
  return (
    <form className="expense-form" onSubmit={handleSubmit}>
      ...
      {/* <Input
        ...
        error={errors.email}
      /> */}
      <button className="add-btn" ref={submitButton}>{editingRowId ? "Save" : "Add"}</button>
    </form>
  )
}